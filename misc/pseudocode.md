...
---
```python
function group_rectangles(rectList, groupThreshold, eps, weights, levelWeights)
    if( groupThreshold <= 0 || rectList.empty() )
        if( weights )
            ...
            for i = 0 to sz - 1
                weights[i] = 1
    nclasses = partition(rectList, labels, similar_rects(eps))
    nlabels = labels.size()
    ...
    if ( levelWeights and weights and not weights.empty() and not levelWeights.empty() )
        for i = 0 to nlabes - 1
            cls = labels[i]
            if( weights[i] > rejectLevels[cls] )
                rejectLevels[cls] = weights[i]
                rejectWeights[cls] = levelWeights[i]
            else if( ( weights[i] == rejectLevels[cls] ) and  levelWeights[i] > rejectWeights[cls]  )
                rejectWeights[cls] = levelWeights)[i]
    else
    ...
    for i = 0 to nclasses - 1
        ...
        for j = 0 to nclasses - 1
            ...
            r2 = rrects[j]
            dx = saturate_cast( r2.width * eps )
            dy = saturate_cast( r2.height * eps )
            if( i != j &&
                r1.x >= r2.x - dx &&
                r1.y >= r2.y - dy &&
