...
void Hough2(IplImage* img, int* circle)
{		
	uchar* pxl_ptr;
	...
	list<voteNode2>*** P = new list<voteNode2>** [x_max];


	int i,j;
	for (i = 0; i < x_max; i++)
	{
		P[i] = new list<voteNode2>*[y_max];
		for (j = 0; j < y_max; j++)
			P[i][j] = new list<voteNode2>[r_max];
	}
...
void findCircles2(IplImage* inputImg, int* circleInfo)
{
